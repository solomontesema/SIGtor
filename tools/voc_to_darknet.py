...
def convert_annotation(dataset_path, year, image_id, output_path, include_difficult):
    ...
    for obj in root.iter('object'):
        ...
        box = (float(xml_box.find('xmin').text), float(xml_box.find('xmax').text), float(xml_box.find('ymin').text),
...
parser = argparse.ArgumentParser(
    description='convert PascalVOC dataset annotation to darknet/YOLOv5 txt annotation files')
parser.add_argument('--dataset_path', type=str, help='path to PascalVOC dataset, default=%(default)s',
                    default=os.getcwd() + '/../../VOCdevkit')
