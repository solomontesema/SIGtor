...
def crop_bbox_image(image_name, bbox, count, class_name, output_path):
    ...
    output_image_name = os.path.join(output_class_path,
                                     os.path.basename(image_name).split('.')[0] + '_' + class_name + '_' + str(
                                         count) + '.jpg')
...
parser.add_argument('--dataset_path', type=str, help='path to PascalVOC dataset, default is ../../VOCdevkit',
                    default=getcwd() + '/../../VOCdevkit')
parser.add_argument('--output_path', type=str,
                    help='output path for croped image and similarity merge txt files, default is ./', default='./')
parser.add_argument('--classes_path', type=str, required=False, help='path to class definitions')
parser.add_argument('--similarity_threshold', type=int, required=False, help='threshold for similarity score',
                    default=0.6)
